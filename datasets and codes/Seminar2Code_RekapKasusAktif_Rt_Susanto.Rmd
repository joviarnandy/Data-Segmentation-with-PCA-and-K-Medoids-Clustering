---
title: "Seminar 2 RekapKasusAktif Rt Susanto"
author: "Jovi Arnandy"
date: "10/24/2020"
...
*read data*
```{r}
normalized_rekapkasus_rt_susanto = read.csv(file="datasets/normalized_data_rekapkasus_rt_susanto.csv")
rownames(normalized_rekapkasus_rt_susanto) = normalized_rekapkasus_rt_susanto$KabupatenKota
normalized_rekapkasus_rt_susanto$KabupatenKota = NULL
```
...
*perform manual pca*
```{r}
df = normalized_rekapkasus_rt_susanto
A = data.matrix(df)
K = (1/(nrow(df)-1)) * t(A) %*% A
```
...
```{r}
clean_data = read.csv(file = "datasets/clean_data_rekapkasus_rt_susanto.csv")
...
points(waktu_penting_ke, y2, col="red", pch="*")
lines(waktu_penting_ke, y2, col="red",lty=2)
legend(5,1500,legend=c("median","rerata"), col=c("blue","red"),pch=c("o","*"),lty=c(1,2), ncol=1)
...
points(waktu_penting_ke, y2, col="red", pch="*")
lines(waktu_penting_ke, y2, col="red",lty=2)
legend(7,4,legend=c("median","rerata"), col=c("blue","red"),pch=c("o","*"),lty=c(1,2), ncol=1)
...
```{r}
Revised_data_gerombol = read.csv("datasets/clean_data_rekapkasus_rt_susanto.csv")
...
leaflet(Revised_data_gerombol) %>%
  ...
  addLabelOnlyMarkers(data = pikobar_df,
                    ...
                    labelOptions = labelOptions(noHide = TRUE, direction = 'top', textOnly = TRUE,style = list("color" = "white", 
                                  ...
                                  "font-style" = "normal",
                                  "font-size" = "12px" ))) %>%
 addLabelOnlyMarkers(108.379537, -6.009965, label =  "Data RekapKasusAktif_Susanto_Penting", 
