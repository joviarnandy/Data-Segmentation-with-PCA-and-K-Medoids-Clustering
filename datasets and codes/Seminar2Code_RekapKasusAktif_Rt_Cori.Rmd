---
title: "Seminar 2 RekapKasusAktif Rt Cori"
author: "Jovi Arnandy"
date: "10/24/2020"
...
*read data*
```{r}
normalized_rekapkasus_rt_cori = read.csv(file="datasets/normalized_data_rekapkasus_rt_cori.csv")
rownames(normalized_rekapkasus_rt_cori) = normalized_rekapkasus_rt_cori$KabupatenKota
normalized_rekapkasus_rt_cori$KabupatenKota = NULL
```
...
*perform manual pca*
```{r}
df = normalized_rekapkasus_rt_cori
A = data.matrix(df)
K = (1/(nrow(df)-1)) * t(A) %*% A
```
...
```{r Cluster Plot}
set.seed(10116070)
res_medoid <- pam(as.data.frame(Ypractical),diss = F,metric ="euclidean",stand = F, k = 3)
...
```{r}
clean_data = read.csv(file = "datasets/clean_data_rekapkasus_rt_cori.csv")
...
points(waktu_penting_ke, y2, col="red", pch="*")
lines(waktu_penting_ke, y2, col="red",lty=2)
legend(3,900,legend=c("median","rerata"), col=c("blue","red"),pch=c("o","*"),lty=c(1,2), ncol=1)
...
points(waktu_penting_ke, y2, col="red", pch="*")
lines(waktu_penting_ke, y2, col="red",lty=2)
legend(2,1500,legend=c("median","rerata"), col=c("blue","red"),pch=c("o","*"),lty=c(1,2), ncol=1)
...
```{r}
Revised_data_gerombol = read.csv("datasets/clean_data_rekapkasus_rt_cori.csv")
...
leaflet(Revised_data_gerombol) %>%
  ...
  addLabelOnlyMarkers(data = pikobar_df,
                    ...
                    labelOptions = labelOptions(noHide = TRUE, direction = 'top', textOnly = TRUE,style = list("color" = "white", 
                                  ...
                                  "font-style" = "normal",
                                  "font-size" = "12px" ))) %>%
 addLabelOnlyMarkers(108.379537, -6.009965, label =  "Data RekapKasusAktif_Cori_Penting", 
